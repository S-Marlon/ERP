...
import { ModalCriarFamiliaRapida } from './ModalCriarFamiliaRapida';
...
export const ItemsConference: React.FC<Props> = ({
  ...
  // 2. Calcula o valor direto na constante
  ...
  // 1. Controle de visibilidade do modal
  const [isModalFamiliaOpen, setIsModalFamiliaOpen] = useState(false);

  // 2. Para abrir o modal (por exemplo, ao clicar num botão "+ Nova Família")
  const handleAbrirModalFamilia = () => {
  ...
  const columns: ColumnsType<Item> = [
    ...
    { title: 'UOM', width: 50, dataIndex: 'unidade', key: 'unidade', render: (text) => <Tag>{text || '-'}</Tag> },
    {
  ...
  return (
    <div style={{ background: '#fff', padding: 0, borderRadius: 8 }}>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 6 }}>
        ...
        <Col>
          <Space>
            ...
            <Button
              type="primary"
              onClick={() => setIsModalFamiliaOpen(true)}
              style={{
                borderRadius: 8,
                fontWeight: 600,
                background: "#2563eb",
      ...
      {/* <ModalCriarFamiliaRapida
visible={isModalFamiliaOpen} // O estado booleano que abre/fecha o modal (true/false)
onCancel={() => setIsModalFamiliaOpen(false)} // Função que fecha o modal
onSuccess={(novaFamilia) => {
// O que acontece quando salva com sucesso:
console.log("Família criada:", novaFamilia);

// Exemplo: Atualizar sua lista de grupos/famílias e já selecionar ela
setGrupos((prev) => [...prev, novaFamilia]);

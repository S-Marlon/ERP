...
const nomeEsperado = typeof gerarPreviewNome === 'function' ? gerarPreviewNome(grupoSelecionado, item.valoresAtributos || {}) : item.nome;
...
if (divergenciaSku && divergenciaNome) {
tipoDivergencia = "⚠️ SKU e Nome divergentes";
} else if (divergenciaSku) {
tipoDivergencia = "⚠️ SKU divergente";
} else if (divergenciaNome) {
tipoDivergencia = "⚠️ Nome divergente";
}

const temDivergencia = Boolean(tipoDivergencia);
...
style={{
padding: "10px",
backgroundColor: temDivergencia ? "#fffbeb" : "#f8fafc",
borderRadius: "6px",
border: `1px solid ${temDivergencia ? "#fde68a" : "#e2e8f0"}`,
}}
>
<div
style={{
...
color: "#64748b",
lineHeight: "1.4",
marginBottom: temDivergencia ? "6px" : "0px",
}}
>
{item.nome}
</div>

{/* Tag de Divergência e Ação Individual */}
{temDivergencia && (
...
{/* Botão de Normalização Individual Cirúrgica */}
{/* Botão de Normalização Individual Cirúrgica */}
<Space size={4}>
  <Button 
    size="small" 
    type="link" 
    style={{ fontSize: "10px", padding: 0, height: "auto", color: "#d97706" }}
    onClick={() => handleTentarNormalizarIndividual(item)}
  >
    Corrigir e Normalizar
  </Button>
  ...
  {divergenciaSku && (
    <Button 
      ...
      type="link" 
      style={{ fontSize: "10px", padding: 0, height: "auto", color: "#d97706" }}
      onClick={() => handleNormalizarItemSku(item)} // Passa o item inteiro em vez de apenas o ID se o seu hook exigir, ou mantenha item.id garantindo escopo local
    >
      Corrigir SKU
    </Button>
  )}
  {divergenciaSku && divergenciaNome && <span style={{ color: "#d97706", fontSize: "10px" }}>|</span>}
  {divergenciaNome && (
    <Button 
      size="small" 
      type="link" 
      style={{ fontSize: "10px", padding: 0, height: "auto", color: "#d97706" }}
      onClick={() => handleNormalizarItemNome(item)} // Passa o item inteiro
    >
      Corrigir Nome
    </Button>
  )}
...
setItensDaFamilia((prevItems: any[]) =>
(prevItems ?? []).map((it) => {
if (it.id === itemEmEdicaoPendencia?.id) {
const novosValores = { ...(it.valoresAtributos || {}), ...values };
...
return { ...it, valoresAtributos: novosValores, sku: novoSku, nome: novoNome };
}
return it;
})
);

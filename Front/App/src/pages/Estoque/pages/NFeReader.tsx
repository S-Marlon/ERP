...
// Ajuste o namespace para o padrão NF-e (xmlns) - FUNDAMENTAL
...
const NFeReader: React.FC = () => {
    ...
    * Busca pelo nome da tag no namespace específico da NFe.
     * @param parent O elemento pai onde a busca deve começar.
     * @param tagName O nome da tag a ser buscada.
     */
   const getTagValue = (parent: Element, tagName: string): string => {
      // Usa getElementsByTagNameNS para lidar corretamente com o namespace
      const element = parent.getElementsByTagNameNS(NFE_NS, tagName)[0];
   ...
   const parseNFeXml = (xmlString: string): NFeData | null => {
      try {
            ...
            // Encontra o nó <infNFe> no namespace da NFe
            const infNFe = xmlDoc.getElementsByTagNameNS(NFE_NS, 'infNFe')[0];
            if (!infNFe) throw new Error('Tag <infNFe> não encontrada. Verifique se o XML é um NFeProc ou NFe válido.');
         ...
         const produtos: Produto[] = Array.from(detElements)
          ...
          // Os campos do produto (cProd, xProd, etc.) são filhos diretos de <prod>.
          // Usamos getTagValue(prod, ...) onde 'prod' é o PAI.
               return {
        ...
        .filter((p): p is Produto => p !== null); // Filtra quaisquer nós nulos
         ...
         return {
            ...
            emitente: {
               ...
               // Encontra o endereço e depois o campo UF
